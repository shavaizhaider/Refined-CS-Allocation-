...
function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[999] flex flex-col gap-2 min-w-[280px]">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={cn(
            'flex items-center gap-3 rounded-xl border px-4 py-3 text-sm font-semibold shadow-xl animate-in slide-in-from-right duration-200',
            t.type === 'success' && 'bg-[#dcebe5] border-[#bcd8cb] text-[#28695e]',
            t.type === 'error' && 'bg-[#fff5f3] border-[#e9beb8] text-[#a8493f]',
...
function Avatar({ name, tone = 'teal' }: { name: string; tone?: 'teal' | 'amber' | 'navy' }) {
  ...
  return <span className={cn('inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-extrabold', colors[tone])}>{initials(name)}</span>;
}
...
function QueryState({ loading, error, onRetry, children, empty = false, emptyText = 'No records found.' }: {
  ...
  if (error) return (
    <div className="border border-[#e9beb8] bg-[#fff5f3] p-5 rounded-xl flex items-center justify-between gap-4">
      <div>
        <p className="font-bold text-[#933f36]">Couldn't load this view.</p>
        <p className="mt-1 text-sm text-[#a8493f]">The backend may not be connected. Try again in a moment.</p>
      </div>
...
function MetricCard({ label, value, note, tone = 'default', icon: Icon, onClick }: {
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        'group relative overflow-hidden rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 transition-all duration-200 hover:-translate-y-0.5',
        onClick && 'cursor-pointer hover:shadow-md hover:border-[hsl(var(--primary))]/50',
        tone === 'alert' && 'border-[#e9beb8] bg-[#fff9f7]',
        tone === 'warm' && 'bg-[#fffaf0]',
      )}
    >
      ...
      <div className="font-mono text-3xl font-medium tracking-[-.06em] text-[hsl(var(--foreground))] flex items-center justify-between">
        <span>{value}</span>
    ...
    <div className="overflow-x-auto rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
      <table className="w-full text-left text-sm" style={{ minWidth }}>
        <thead className="border-b border-[hsl(var(--border))] bg-[hsl(var(--muted))]/50">
          <tr>
            {headers.map((h) => <th key={h} className="whitespace-nowrap px-4 py-3 font-mono text-[10px] font-medium uppercase tracking-[.12em] text-[hsl(var(--muted-foreground))]">{h}</th>)}
          </tr>
        </thead>
...
function PageHeader({ eyebrow, title, description, actions }: { eyebrow: string; title: string; description: string; actions?: ReactNode }) {
  return (
    <div className="mb-8 pt-2 flex flex-col justify-between gap-5 md:flex-row md:items-end">
      <div>
        <p className="font-mono text-[10px] font-medium uppercase tracking-[.2em] text-[hsl(var(--primary))]">{eyebrow}</p>
        <h1 className="mt-2 font-display text-[clamp(2rem,3vw,3.2rem)] leading-[.95] tracking-[-.045em] text-[hsl(var(--foreground))]">{title}</h1>
...
// ─── Shell / Navigation & Active Cycle Widget ─────────────────────────────
    ...
    <div className="noise min-h-[100dvh] bg-[hsl(var(--background))]">
      <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-[272px] flex-col border-r border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar))] px-4 py-5 text-[hsl(var(--sidebar-foreground))] transition-transform duration-200 lg:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
        <div className="flex items-center justify-between px-3 pb-8">
          ...
          <button onClick={() => setMobileOpen(false)} className="rounded-md p-2 text-[#aeb7bd] hover:bg-[hsl(var(--sidebar-accent))] lg:hidden"><X size={17} /></button>
        </div>

        {/* ── Active Cycle Widget (Clickable & Switchable) ── */}
        <div
          onClick={() => setShowCycleModal(true)}
          className="mb-6 cursor-pointer rounded-xl border border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar-accent))] p-3 transition-all hover:border-[#387c71] hover:bg-[#1a2c38]"
        >
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                      className={cn('group flex items-center rounded-lg px-3 py-2.5 text-[13px] font-semibold transition-colors gap-3',
      ...
      {mobileOpen && <button aria-label="Close navigation" className="fixed inset-0 z-30 bg-[#13212b]/45 lg:hidden" onClick={() => setMobileOpen(false)} />}

      {/* ── Active Cycle Switcher Modal ── */}
      {showCycleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#13212b]/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="w-full max-w-md rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-base">Select Academic Cycle</h3>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isCurrent ? 'border-[hsl(var(--primary))] bg-[#eef6f2]' : 'border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))]/50',
                    )}
                  >
                    <div>
...
function Dashboard() {
  ...
  // Affected faculty for slide-over drawer
  ...
  return (
    <>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="rounded-md bg-[#dcebe5] px-2 py-0.5 text-[10px] font-extrabold text-[#28695e]">0.0% Collisions</span>
              </div>
            ...
            <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="rounded-md bg-[#dcebe5] px-2 py-0.5 text-[10px] font-extrabold text-[#28695e]">98% Aligned</span>
              </div>
            ...
            <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="rounded-md bg-[#dcebe5] px-2 py-0.5 text-[10px] font-extrabold text-[#28695e]">VERIFIED</span>
              </div>
                  ...
                  programmes.map((item) => {
                    ...
                    return (
                      <div
                        ...
                        className="group cursor-pointer rounded-lg p-2 transition-colors hover:bg-[hsl(var(--muted))]/40"
                      >
              ...
              <div className="grid grid-cols-2 gap-2">
                {([
                  ...
                  <div
                    ...
                    className="cursor-pointer rounded-lg border border-[hsl(var(--border))] p-3 transition-all hover:border-[hsl(var(--primary))] hover:shadow-sm"
                  >
                    <div className={cn('mb-4 inline-flex rounded-md px-2 py-1 text-[10px] font-bold', cls)}>{label}</div>
      ...
      {/* Affected Faculty Slide-Over Drawer */}
      {selectedLoadFilter && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md h-full bg-[hsl(var(--card))] border-l border-[hsl(var(--border))] p-6 shadow-2xl overflow-y-auto animate-in slide-in-from-right">
            <div className="flex items-center justify-between mb-6 border-b border-[hsl(var(--border))] pb-4">
              <div>
                <h3 className="font-extrabold text-base">{selectedLoadFilter} Faculty</h3>
            ...
            <div className="space-y-3">
              {affectedFaculty.map((f) => (
                <div key={f.id} className="rounded-xl border border-[hsl(var(--border))] p-4">
...
// ─── Open Conflicts Resolution System Page ─────────────────────────────────────
        ...
        {(
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === tabKey ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--card))] border border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))]',
            )}
          >
            {label}
          </button>
      ...
      <QueryState loading={dashboard.isLoading} error={dashboard.error} onRetry={() => dashboard.refetch()} empty={!filteredConflicts.length} emptyText="No active allocation conflicts detected! All assignments meet department criteria.">
        <div className="space-y-4">
          {filteredConflicts.map((c) => (
            <div key={c.id} className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-start gap-4">
                <div className={cn('mt-0.5 rounded-lg p-2.5', c.severity === 'CRITICAL' ? 'bg-[#f7e3df] text-[#a8493f]' : 'bg-[#f7ecd0] text-[#8b681d]')}>
                  <ShieldAlert size={20} />
                </div>
                <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-extrabold text-base">HOD Force-Approve Override</h3>
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-[hsl(var(--border))] bg-white p-3 text-xs outline-none focus:border-[hsl(var(--primary))]"
            />
...
function CoursesPage() {
  ...
  const [form, setForm] = useState({ code: '', title: '', programme: 'BSCS', semester: '1', credit: '3(3,0)', theory: 3, lab: 0, category: 'Core', domain: 'Software Engineering' });
  const courses = useListCourses({ search: search || undefined }, { query: { queryKey: getListCoursesQueryKey({ search: search || undefined }) } });
  const create = useCreateCourse();
  const qc = useQueryClient();
  const [, setLocation] = useLocation();

  const resetForm = () => { setForm({ code: '', title: '', programme: 'BSCS', semester: '1', credit: '3(3,0)', theory: 3, lab: 0, category: 'Core', domain: 'Software Engineering' }); setAdding(false); };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    create.mutate(
      ...
      {
        onSuccess: () => {
          toast(`Course "${form.code.toUpperCase()}" added successfully!`, 'success');
          resetForm();
  ...
  return (
    <>
      <PageHeader
        ...
        title="Real CS Course Catalogue"
        description="Official computer science department courses with domain tagging and prerequisite tracking."
        actions={
      ...
      <Toolbar>
        <SearchBar value={search} onChange={setSearch} placeholder="Search course code, title, or domain…" testId="input-course-search" />
      ...
      {adding && (
        <form onSubmit={handleSubmit} className="mb-4 rounded-xl border border-[hsl(var(--primary))]/30 bg-[#eef6f2] p-5" data-testid="form-add-course">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="font-extrabold text-sm">Add New Course</h3>
            <button type="button" onClick={resetForm}><X size={17} /></button>
          </div>
          <div className="grid gap-3 md:grid-cols-4">
            <input required value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="e.g. CSC301" className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none focus:border-[hsl(var(--primary))]" />
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Operating Systems" className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none focus:border-[hsl(var(--primary))] md:col-span-2" />
            <select value={form.programme} onChange={(e) => setForm({ ...form, programme: e.target.value })} className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none">
          ...
          <div className="grid gap-3 md:grid-cols-4 mt-3">
            <select value={form.semester} onChange={(e) => setForm({ ...form, semester: e.target.value })} className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none">
              {['1','2','3','4','5','6','7','8'].map((s) => <option key={s} value={s}>Semester {s}</option>)}
            </select>
            <input value={form.credit} onChange={(e) => setForm({ ...form, credit: e.target.value })} placeholder="e.g. 3(3,0)" className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none" />
            <select value={form.domain} onChange={(e) => setForm({ ...form, domain: e.target.value })} className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none">
              <option value="Software Engineering">Software Engineering</option>
              <option value="AI & Data Science">AI & Data Science</option>
              <option value="Computer Networks">Computer Networks</option>
              <option value="Cyber Security">Cyber Security</option>
            </select>
            <input type="number" value={form.theory} onChange={(e) => setForm({ ...form, theory: Number(e.target.value) })} placeholder="Theory hrs" className="border border-[hsl(var(--border))] bg-white px-3 py-2 text-xs rounded-lg outline-none" />
          </div>
      ...
      <QueryState loading={courses.isLoading} error={courses.error} onRetry={() => courses.refetch()} empty={!courses.data?.length} emptyText={search ? 'No courses match that search.' : 'No courses found. Add one above.'}>
        <DataTable headers={['Code', 'Course Title', 'Programme', 'Semester', 'Credit', 'Domain Tag', 'Status', 'Actions']} minWidth="900px">
          {courses.data?.map((course: any) => (
            <tr key={course.id} className="group transition-colors hover:bg-[hsl(var(--muted))]/50">
              <td className="px-4 py-3.5">
                <button onClick={() => setSelectedCourse(course)} className="font-mono text-xs font-bold text-[hsl(var(--primary))] hover:underline flex items-center gap-1">
                  {course.code} <Info size={12} />
                </button>
              </td>
              ...
              <td className="px-4 py-3.5">
                <div className="flex items-center gap-1">
                  <button title="Allocate Sections" onClick={() => setLocation('/allocation')} className="p-1.5 rounded-lg text-[hsl(var(--primary))] hover:bg-[#eef6f2] text-xs font-bold">
                    Allocate
                  </button>
                  <button title="Delete course" onClick={() => handleDeleteCourse(course.id, course.code)} className="p-1.5 rounded-lg text-[#a8493f] hover:bg-[#fff5f3] transition-colors">
                    <Trash2 size={13} />
                  </button>
                </div>
              </td>
      ...
      {/* Course Detail Quick-View Drawer */}
      {selectedCourse && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4 border-b pb-3">
              <div>
                <h3 className="font-extrabold text-base">{selectedCourse.code} — {selectedCourse.title}</h3>
                <p className="text-xs text-[hsl(var(--muted-foreground))]">{selectedCourse.programme} Semester {selectedCourse.semester}</p>
              </div>
              <button onClick={() => setSelectedCourse(null)}><X size={18} /></button>
            </div>
            <div className="space-y-4 text-xs">
              <div>
                <p className="font-bold text-[hsl(var(--muted-foreground))]">Domain Specialization</p>
                <p className="font-bold text-sm mt-1">{selectedCourse.domain || 'Software Engineering'}</p>
              </div>
              <div>
                <p className="font-bold text-[hsl(var(--muted-foreground))]">Prerequisites</p>
                <p className="font-mono mt-1">{selectedCourse.prerequisites?.length ? selectedCourse.prerequisites.join(', ') : 'None (Entry level)'}</p>
              </div>
              <div>
                <p className="font-bold text-[hsl(var(--muted-foreground))]">Credit Structure</p>
                <p className="font-mono mt-1">{selectedCourse.credit} ({selectedCourse.theory}h theory, {selectedCourse.lab}h lab)</p>
              </div>
            </div>
            <div className="mt-6 flex justify-end">
              <Button testId="button-allocate-course-direct" onClick={() => { setSelectedCourse(null); setLocation('/allocation'); }}>Allocate Sections in Workspace</Button>
            </div>
          </div>
        </div>
      )}
...
// ─── Faculty Page (with View Switcher & Live Workload Progress Bars) ───────────

function FacultyPage() {
  const [search, setSearch] = useState('');
  const [typeFilter, setTypeFilter] = useState('');
  const [viewMode, setViewMode] = useState<'GRID' | 'TABLE'>('GRID');
  const [adjustFaculty, setAdjustFaculty] = useState<any | null>(null);
  const [newMaxLoad, setNewMaxLoad] = useState('12');
  ...
  const handleExport = () => {
    ...
    exportToExcel(
      filtered.map((f) => ({ Name: f.name, Designation: f.designation, Type: f.type, Programme: f.programme, 'Current Load': f.currentLoad, 'Max Load': f.maximumLoad, Status: f.status })),
  ...
  const handleAdjustLoad = () => {
    if (adjustFaculty) {
      adjustFaculty.maximumLoad = newMaxLoad;
      toast(`Updated max workload limit for ${adjustFaculty.name} to ${newMaxLoad}h`, 'success');
      setAdjustFaculty(null);
  ...
  return (
    <>
      <PageHeader
        ...
        description="Permanent & Visiting Faculty with live workload progress indicators."
        actions={
      ...
      <Toolbar>
        ...
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="px-3 py-1.5 text-xs font-bold rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] outline-none">
          <option value="">All Types</option>
          <option value="Permanent">Permanent</option>
          <option value="Visiting">Visiting</option>
        </select>
        <div className="flex items-center gap-1 border border-[hsl(var(--border))] rounded-lg p-1 bg-[hsl(var(--card))]">
          <button onClick={() => setViewMode('GRID')} className={cn('p-1 rounded', viewMode === 'GRID' && 'bg-[hsl(var(--primary))] text-white')}><Grid size={14} /></button>
          <button onClick={() => setViewMode('TABLE')} className={cn('p-1 rounded', viewMode === 'TABLE' && 'bg-[hsl(var(--primary))] text-white')}><TableIcon size={14} /></button>
        </div>
        ...
        {viewMode === 'GRID' ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((person: any) => {
              ...
              return (
                <div key={person.id} className="group rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 transition-all hover:border-[hsl(var(--primary))]/45">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      <Avatar name={person.name} tone={person.type === 'Visiting' ? 'amber' : 'teal'} />
                      <div>
                        <h3 className="font-extrabold text-sm">{person.name}</h3>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <button onClick={() => { setAdjustFaculty(person); setNewMaxLoad(String(max)); }} className="font-mono font-bold hover:underline">
                        {current}/{max}h Limit
                      </button>
                    </div>
          ...
          <DataTable headers={['Faculty Name', 'Designation', 'Type', 'Department', 'Load Progress', 'Status']} minWidth="800px">
            {filtered.map((person: any) => (
              <tr key={person.id} className="hover:bg-[hsl(var(--muted))]/50">
                ...
                <td className="px-4 py-3.5 text-xs">{person.department || 'Computer Science'}</td>
                <td className="px-4 py-3.5 font-mono text-xs font-bold">{person.currentLoad}/{person.maximumLoad}h</td>
                <td className="px-4 py-3.5"><StatusPill status={person.status} /></td>
              </tr>
            ))}
          </DataTable>
        )}
      ...
      {/* Adjust Max Workload Limit Modal */}
      {adjustFaculty && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            <h3 className="font-extrabold text-base">Adjust Max Workload Limit</h3>
            <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Change maximum allowed credit hours for <strong>{adjustFaculty.name}</strong>.</p>
            <input
              type="number"
              value={newMaxLoad}
              onChange={(e) => setNewMaxLoad(e.target.value)}
              className="mt-4 w-full rounded-xl border border-[hsl(var(--border))] bg-white p-3 font-mono text-sm outline-none"
            />
            <div className="mt-5 flex justify-end gap-2">
              <Button variant="outline" testId="button-cancel-adjust" onClick={() => setAdjustFaculty(null)}>Cancel</Button>
              <Button testId="button-save-adjust" onClick={handleAdjustLoad}>Save Limit</Button>
            </div>
          </div>
        </div>
      )}
...
// ─── Allocation Workspace (with Smart Recommendations & Bulk Actions) ──────────
...
function AllocationPage() {
  ...
  const assign = async (offering: Offering, value: string) => {
    try {
      const token = localStorage.getItem('cs_token');
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      ...
      if (res.ok) {
        ...
        toast(`${offering.courseCode} assigned to ${value || 'Unassigned'}`, 'success');
        qc.invalidateQueries({ queryKey: getListOfferingsQueryKey() });
        qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() });
  ...
  const filtered = (offerings.data ?? []).filter((o) => {
    ...
    const matchProg = !progFilter || o.programme === progFilter;
  ...
  return (
    <>
      <PageHeader
        ...
        description="Assign faculty instructors to real FA25 section offerings with smart recommendations & bulk actions."
      />
      <Toolbar>
        <SearchBar value={search} onChange={setSearch} placeholder="Search course code or title…" />
        <select value={progFilter} onChange={(e) => setProgFilter(e.target.value)} className="px-3 py-1.5 text-xs font-bold rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] outline-none">
          <option value="">All Programmes</option>
          <option value="BSCS">BSCS</option>
          <option value="BSSE">BSSE</option>
          <option value="MSCS">MSCS</option>
        </select>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-1.5 text-xs font-bold rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] outline-none">
      ...
      {selectedIds.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[hsl(var(--primary))]/30 bg-[#eef6f2] p-3 text-xs font-bold">
          <span>{selectedIds.length} sections selected</span>
          <div className="flex items-center gap-2">
            <select value={bulkFaculty} onChange={(e) => setBulkFaculty(e.target.value)} className="rounded-lg border border-gray-300 bg-white px-2 py-1 outline-none">
              <option value="">Select Faculty for Bulk Assign</option>
              {faculty.data?.map((p) => <option key={p.id} value={p.name}>{p.name}</option>)}
            </select>
            <Button testId="button-bulk-assign" onClick={handleBulkAssign}>Bulk Assign</Button>
          </div>
        </div>
      )}
          ...
          {filtered.map((offering) => {
            ...
            return (
              <tr key={offering.id} className="hover:bg-[hsl(var(--muted))]/50 transition-colors">
                ...
                <td className="px-4 py-3.5 font-bold text-xs max-w-[160px]"><span className="line-clamp-2">{offering.courseTitle}</span></td>
                <td className="px-4 py-3.5 text-xs">{offering.programme}</td>
                <td className="px-4 py-3.5 font-mono text-xs whitespace-nowrap">Sem {offering.semester} — {offering.section}</td>
                ...
                <td className="px-4 py-3.5 min-w-[180px]">
                  <select
                    ...
                    className="w-full max-w-[200px] min-h-8 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-2 py-1 text-xs font-bold outline-none focus:border-[hsl(var(--primary))] cursor-pointer"
                  >
...
// ─── Planning Page (with Carry Over / Lock Cycle Tools) ───────────────────────
...
function PlanningPage() {
  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center gap-2 border-b pb-3">
        {(['ALL', 'ACTIVE', 'DRAFTS', 'ARCHIVED'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn('px-3.5 py-1.5 text-xs font-bold rounded-lg', tab === t ? 'bg-[hsl(var(--primary))] text-white' : 'bg-[hsl(var(--card))] border')}
          >
            {t}
          </button>
        ))}
      ...
      <QueryState loading={sessions.isLoading} error={sessions.error} onRetry={() => sessions.refetch()} empty={!filteredSessions.length} emptyText="No sessions found.">
        <div className="space-y-4">
          {filteredSessions.map((s: any) => {
            ...
            return (
              <div key={s.id} className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="font-display text-2xl font-bold">{s.code} — {s.label}</h2>
                    {s.locked && <span className="flex items-center gap-1 rounded-full bg-gray-200 px-2.5 py-0.5 text-[10px] font-bold text-gray-700"><Lock size={11} /> LOCKED</span>}
                  </div>
                  <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{s.term} {s.year} · Micro-stats: 59 Offerings · 70 Faculty</p>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={() => toggleLock(s)} title="Toggle Lock Cycle" className="p-2 rounded-lg border hover:bg-gray-100">
      ...
      {cloneModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border bg-[hsl(var(--card))] p-6 shadow-2xl">
            <h3 className="font-extrabold text-base">Carry Over / Clone Academic Cycle</h3>
            ...
            <div className="mt-4 space-y-3">
              <input value={cloneCode} onChange={(e) => setCloneCode(e.target.value)} placeholder="e.g. SP26" className="w-full rounded-xl border p-3 text-xs outline-none" />
              <input value={cloneLabel} onChange={(e) => setCloneLabel(e.target.value)} placeholder="e.g. Spring 2026 Cycle" className="w-full rounded-xl border p-3 text-xs outline-none" />
            </div>
            <div className="mt-5 flex justify-end gap-2">
